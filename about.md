...
이를 해결하기 위해 raw 기반의 .csv가 아닌 column 기반의 parquet을 채택했다. 
...
당시 가상화 데이터를 불러오는 서비스에서 메타데이터 관리 서비스를 거쳐 필요한 데이터를 가져와 물리적인 데이터로 변경을 하는 로직이 존재했는데 물리적인 데이터로 변환하는 부분이 아닌 메타데이터 관리 서비스를 거치는 부분에서 오버헤드가 발생하는 것이 모니터링 됐었다.
